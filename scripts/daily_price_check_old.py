...
if new_trades:
    out_dir = os.path.dirname(data_path)
    filename = f"new_trade_{today.strftime('%Y_%m_%d')}.json"
    out_path = os.path.join(out_dir, filename)
...
for t in new_trades:
    cur.execute('''
        ...
        ORDER BY deal_amount DESC LIMIT 1
    ''', (t['lawd_cd'], t['apt_name'], t['exclu_use_ar']))
    old_record = cur.fetchone()
    
    if old_record:
        old_max = old_record[0]
        if t['deal_amount'] > old_max:
            rate = (t['deal_amount'] - old_max) / old_max * 100
            t['old_max'] = old_max
...
if ath_list:
    filename = f"ath_list_{today.strftime('%Y_%m_%d')}.json"
...
def generate_ranking_image(top_data, title, out_filename):
    ...
    draw_c(f"({today.strftime('%Y년 %m월 %d일')} 업데이트 기준, 전용 면적)", w/2, 270, font_sub, (200, 200, 200))

    # Table Headers
    cols = [(70, "순위"), (310, "아파트 (지역)"), (610, "신규거래(일자)"), (820, "이전최고(일자)"), (990, "상승률")]
    y_start = 380
    ...
    row_h = 135
    if not top_data:
        draw_c("신고가 거래 없음", w/2, y + 200, font_title, (180, 180, 180))
    for i, item in enumerate(top_data, 1):
        ...
        # Old Deal
        draw_c(format_price(item['old_max']), cols[3][0], y - 20, font_row, (200, 200, 200))
...
# Ranking 1: 상승률 기준 (Rate - Descending)
ath_list_sorted_rate = sorted(ath_list, key=lambda x: x['rate'], reverse=True)
generate_ranking_image(ath_list_sorted_rate[:10], "오늘의 신고가 상승률 TOP10", "/Users/smithkwon/.openclaw/workspace/dailypriceup/data/daily_shorts_rate.png")

# Ranking 2: 거래 금액 기준 (Amount - Descending)
ath_list_sorted_highest = sorted(ath_list, key=lambda x: x['deal_amount'], reverse=True)
generate_ranking_image(ath_list_sorted_highest[:10], "오늘의 신고가 거래금상위 TOP10", "/Users/smithkwon/.openclaw/workspace/dailypriceup/data/daily_shorts_amount.png")

# Ranking 3: 거래 금액 작은 순 기준 (Amount - Ascending)
ath_list_sorted_lowest = sorted(ath_list, key=lambda x: x['deal_amount'], reverse=False)
generate_ranking_image(ath_list_sorted_lowest[:10], "오늘의 신고가 거래금 하위 TOP10", "/Users/smithkwon/.openclaw/workspace/dailypriceup/data/daily_shorts_lowest.png")
...
def generate_general_ranking_image(top_data, title, out_filename):
    ...
    # Table Headers
    cols = [(70, "순위"), (310, "아파트 (지역)"), (590, "층수 (면적)"), (790, "거래금액"), (990, "거래일자")]
    y_start = 380
    draw.line([(40, y_start-30), (w-40, y_start-30)], fill=(100, 100, 150), width=3)
    for cx, cname in cols:
        draw_c(cname, cx, y_start, font_col, (150, 180, 255))
...
# 항상 오늘 전체 실거래 TOP10 추가
new_trades_sorted = sorted(new_trades, key=lambda x: x['deal_amount'], reverse=True)
generate_general_ranking_image(new_trades_sorted[:10], "오늘의 실거래 TOP10", "/Users/smithkwon/.openclaw/workspace/dailypriceup/data/daily_shorts_top10.png")
